...
Base-training variant of CONCIL. For paper reproduction use CONCIL_1114.py.
"""
import os
import sys
...
# CONCIL analytic modules (RecursiveLinear, Buffer, Learner)
...
from src.analytic.Learner import Learner, loader_t
from src.analytic.AnalyticLinear import AnalyticLinear, RecursiveLinear
...
class CONCIL(nn.Module):
    def __init__(self, num_concepts: int, num_classes: int,
        ...
        # Backbone feature extractor and output dimension
        self.backbone = backbone
        self.backbone_output = backbone_output

        # RandomBuffer for storing backbone outputs
        ...
        self.buffer_class = RandomBuffer(self.num_concepts, buffer_size, **factory_kwargs)

        if vision_backbone == 'resnet':
            self.backbone = resnet50(weights=ResNet50_Weights.DEFAULT)
            self.concept_fc = nn.Linear(1000, num_concepts).to(self.device)
        elif vision_backbone == 'vit':
            self.concept_encoder = ViTModel.from_pretrained('google/vit-base-patch16-224-in21k')
            self.concept_fc = nn.Linea(1000, num_concepts).to(self.device)
        ...
        ## CONCIL
        # Analytic linear layers (RecursiveLinear by default)
        ...
        # No further training; updates are analytic (closed-form)
        self.eval()

    ## Extract features via backbone and pass to buffer for expansion.
    @torch.no_grad()
    ...
    ## Pass features to analytic linear layer
    @torch.no_grad()
    ...
    # Call analytic layer update (internal parameter update).
    @torch.no_grad()
    def update(self) -> None:
        self.analytic_linear_concept.update()
        self.analytic_linear_class.update()

    ## Fit (closed-form solve)
    @torch.no_grad()
    def fit_concept_linear(self, X: torch.Tensor, y: torch.Tensor, *args, **kwargs) -> None:
        ...
        # print(y)
        # Convert labels y to one-hot Y
        # Y = torch.nn.functional.one_hot(y)
        Y = y
        # Feature expansion for input X
        # print("111111")
        ...
        # pdb.set_trace()  # breakpoint        
        # print("2222222")
        # Fit analytic layer with expanded X and one-hot Y.
...
class IncrementalIMGDataset(Dataset):
    def __init__(self, data_path: str, split: str, resol: int = 256, class_ratio: float = 0.5, prev_class_ratio: float = 0.0, conecpt_ratio: float = 0.5):
        assert split in ['train', 'test']
        self.data = read_data(data_path, split)
        if 'cub' in data_path:
            self.n_class = CONFIG['cub']['N_CLASSES']
...
def get_incremental_dataloader(dataset, data_path, batch_size, class_ratio=0.5, prev_class_ratio=0.0, conecpt_ratio=0.5):
    train_loader = DataLoader(dataset=IncrementalIMGDataset(data_path, split='train', class_ratio=class_ratio, prev_class_ratio=prev_class_ratio, conecpt_ratio=conecpt_ratio), batch_size=batch_size, shuffle=True, num_workers=16)
    test_loader = DataLoader(dataset=IncrementalIMGDataset(data_path, split='test', class_ratio=class_ratio, prev_class_ratio=prev_class_ratio, conecpt_ratio=conecpt_ratio), batch_size=batch_size, shuffle=True, num_workers=16)
    return train_loader, test_loader
...
class IncrementalIMGTrainer(BASETrainer):
    ...
    def expand_model(self, stage):
        ...
        if stage == 0:
            ...
            new_num_concepts = int(self.num_concepts*self.args.conecpt_ratio + stage*(1-self.args.conecpt_ratio)/(self.num_stages-1)*self.num_concepts)
    ...
    def loss(self, concept_pred, concept, label_pred, label, concept_lambda, num_concepts, stage):
        ...
        # print("concept_subset.shape:",concept_subset.shape)
        # print("concept_pred:",concept_pred)
        # print("concept_pred.shape:",concept_pred.shape)
        if stage == 0:
        ...
        # print("Concept_GT")
        # print(concept_subset)
        # print(concept_subset.shape)
        # print("label")
        # print(label_pred)
    ...
    def train_incrementally(self):
        ...
        conecpt_ratio = self.args.conecpt_ratio
        for stage in range(1):
            self.plog(f'Starting Incremental Stage {stage+1}')
            # print("stage:",stage)
            # self.load_model(stage)
            # self.expand_model(stage)
            self.train_loader, self.test_loader = get_incremental_dataloader(self.dataset, self.data_path, self.batch_size, class_ratio=class_ratio, prev_class_ratio=prev_class_ratio, conecpt_ratio=conecpt_ratio)
            # print("train--prev_class_ratio:",prev_class_ratio)
            # print("train--class_ratio:",class_ratio)
            # print("train--conecpt_ratio:",conecpt_ratio)
            ...
            ### CONCIL phase (analytic updates)
            ###################
            if stage == 0:
            # Base phase: standard training
                epochs = 50
            ...
            #     path_to_checkpoint = "/hpc2hdd/home/songninglai/CBM_CL/results/2024-11-12-18:11:28/ckpt_stage_1_epoch_5.pth"
            #     self.model.load_state_dict(torch.load(path_to_checkpoint))
            #     self.model.eval()
            #     print("base training via CONCIL.....")
            #     epoch = 0
            #     self.model.eval()
            #     self.backbone = self.model.backbone
            #     self.backbone_output = 1000
            #     self.buffer_size = args.buffer_size
            #     self.model1 = CONCIL(
            #         backbone_output=self.backbone_output,
            ...
            #         vision_backbone = args.v_backbone
            #     )
            ...
            #         ###
            #         # concept_pred, label_pred, hidden_output = self.model(img, num_concepts=self.model.num_concepts)
            #         # print(self.model.num_concepts)
            #         ###
            #         # concept_pred, label_pred, hidden_output = self.model1(img, num_concepts=self.model.num_concepts, use_acil=True)
            #         self.model1.fit_concept_linear(img, concept)
            #         # self.model1.fit_concept_linear(img, concept)
            #         # concept_pred_concil = self.model1.forward_concept(img)
            #         # self.model1.fit_class_linear(concept_pred_concil,label)
            #         # self.model1.fit_class_linear(concept, label) 
            #         # self.model1.update()
            #         ### 2
            #         self.model1.fit_concept_linear(img, concept)
            #     self.model1.update()
            #         # concept_pred,label_pred = self.model1(img)
            #         #loss = self.loss(concept_pred, concept, label_pred, label, concept_lambda=self.args.concept_lambda, num_concepts=self.model.num_concepts,stage=stage)
            #         #epoch_loss.append(loss.item())
            #     #avg_epoch_loss = sum(epoch_loss) / len(epoch_loss)
            #     #self.train_losses.append(avg_epoch_loss)
            ...
            # else:
            # ## Later phases: CONCIL (analytic) updates
            #     # epoch = 0
            #     # self.model.eval()
            #     # self.backbone = self.model.backbone
            #     # self.backbone_output = 1000
        ...
        with open(csv_path, mode='w', newline='') as csvfile:
            ...
            for prev_stage in range(stage + 1):
                ...
                if prev_stage == 0:
                    ...
                    conecpt_ratio = self.args.conecpt_ratio + (prev_stage)*(1-self.args.conecpt_ratio)/(self.num_stages-1)
                    # print("test--prev_class_ratio:",prev_class_ratio)
                    # print("test--class_ratio:",class_ratio)
                    # print("test--conecpt_ratio:",conecpt_ratio)
                
                test_loader = DataLoader(dataset=IncrementalIMGDataset(self.data_path, split='test', class_ratio=class_ratio, prev_class_ratio=prev_class_ratio, conecpt_ratio=conecpt_ratio), batch_size=self.batch_size, shuffle=True, num_workers=16)
                    ...
                    with torch.no_grad():
                        if prev_stage == 0:
                            if hasattr(self, 'model1'):
                                # print("CONCIL Testing")
                                concept_pred,label_pred = self.model1(img)
                                concept_subset = concept[:, :self.model.num_concepts]
                                concept_pred = concept_pred[:, :self.model.num_concepts]
                            else:
                                concept_pred, label_pred, _ = self.model(img, num_concepts=self.model.num_concepts)
                                concept_subset = concept[:, :self.model.num_concepts]
                            # print(concept[:, :self.model.num_concepts].shape)
                            # print(concept.shape)
                ...
                # print(f'Task {prev_stage + 1} Concept Predictions: {concept_pred.ge(0.5)}')
                # print(f'Task {prev_stage + 1} True Concepts: {concept_subset}')
                ...
                if prev_stage == stage:
                    test_loader = DataLoader(dataset=IncrementalIMGDataset(self.data_path, split='test', class_ratio=class_ratio, prev_class_ratio=0, conecpt_ratio=conecpt_ratio), batch_size=self.batch_size, shuffle=True, num_workers=16)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-epoch', '-e', type=int, default=5, help='epochs for each training stage')
    ...
    parser.add_argument('-class_ratio', type=float, default=1, help='initial ratio of classes used')
    parser.add_argument('-conecpt_ratio', type=float, default=1, help='initial ratio of concepts used')
    parser.add_argument('-concept_ratio', type=float, default=1, help='initial ratio of concepts used')
    parser.add_argument('-num_stages', type=int, default=2, help='number of incremental stages')
    parser.add_argument('-buffer_size', type=int, default=8192*10, help='buffer_size')
    parser.add_argument('-gg', type=float, default=1e-1, help='nnnn')
    parser.add_argument('-gg1', type=float, default=1e-1, help='concept')
    parser.add_argument('-gg2', type=float, default=1e-1, help='class')


    
    # parser.add_argument('-mode', type=int, default='CL', help='CL/CONCIL')
    args = parser.parse_args()
