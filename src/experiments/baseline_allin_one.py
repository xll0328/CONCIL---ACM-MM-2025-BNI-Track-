...
class IMGTrainer(BASETrainer):
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            self.plog(f'Epoch {epoch} training loss: {avg_epoch_loss}')
            clf_accu = self.test()
    ...
    def test(self):
        ...
        self.plog(f'epoch testing loss: {avg_loss}')
    ...
    def _plot_training_curves(self):
        ...
        plt.savefig(join(self.args.saved_dir, self.time, 'loss_curves.png'))
        plt.close()

        plt.figure()
        plt.plot(epochs, self.test_accus, label='Testing Accuracy')
        plt.xlabel('Epochs')
        plt.ylabel('Accuracy')
        plt.legend()
        plt.title('Testing Accuracy Curve')
        plt.savefig(join(self.args.saved_dir, self.time, 'accuracy_curve.png'))
        plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-epoch', '-e', type=int, default=2, help='epochs for training process')
