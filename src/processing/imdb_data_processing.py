...
def read_csv_data(data_dir):
    ...
    cwd = os.getcwd()
    train1 = pd.read_csv(join(cwd, data_dir, 'IMDB-train-manual.csv'))
    train2 = pd.read_csv(join(cwd, data_dir, 'IMDB-train-generated.csv'))
    val1 = pd.read_csv(join(cwd, data_dir, 'IMDB-dev-manual.csv'))
    val2 = pd.read_csv(join(cwd, data_dir, 'IMDB-dev-generated.csv'))
    test1 = pd.read_csv(join(cwd, data_dir, 'IMDB-test-manual.csv'))
    test2 = pd.read_csv(join(cwd, data_dir, 'IMDB-test-generated.csv'))
    ...
    attribute_name = train.columns[2: ] ## save the last attribute, bit different from the paper

    def fn1(x):
        return x.replace('Negative ', 'Negative')
    def fn2(x):
        return x.replace('Unkown', 'unknown')

    train = train.apply(fn1).apply(fn2)
    val = val.apply(fn1).apply(fn2)
    test = test.apply(fn1).apply(fn2)
    
    return train, val, test, attribute_name 

def create_new_data(data: pd.DataFrame):

    attribute_map = {'Negative': 0, 'Positive': 1, 'unknown': 2}
    ...
    for i in range(len(data)):
        ...
        new_instance['label'] = label_map[instance[label_column]]
        for attribute in columns[2: ]:
...
if __name__ == "__main__":
    ...
    with open('src/utils/data_path.yml', 'a') as f:
        yaml.dump({'imdb': path}, f, default_flow_style = False)